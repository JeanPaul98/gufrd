...
/**
 * @author Olivier on 19/08/2024
 * @project formalite-agriculture-api
 */
@Service
public class AutorisationConsommationClientCRUDServiceImpl implements AutorisationConsommationLocalClientCRUDService {

    private final Logger logger = LoggerFactory.getLogger(AutorisationConsommationClientCRUDServiceImpl.class);
    ...
    public AutorisationConsommationClientCRUDServiceImpl(AutorisationRepository autorisationRepository,
                                                         FormaliteRepository formaliteRepository, PieceJointeRepository pieceJointeRepository, TypePieceJointeRepository typePieceJointeRepository,
    ...
    public ResponseEntity<?> create(AutorisationImportationDto request) {
        ...
        for (DetAutorisationProduitDto detAutorisationProduitDto : request.getDetAutorisationProduitDtos()) {
            ...
            logger.info("Nom du produt {} ",detAutorisationProduitDto.getProduit().getCode());
    ...
    public ResponseEntity<?> saveConsommationFormalite(String numeroAgrement, Autorisation autorisation, String compteClient,
        ...
        if(agrement.isPresent()) {
            ...
            formalite.setNumGenere(utilServices.genererNumeroDemande(env.getProperty("message.code.genere.numero.autorisation")));
    ...
    private Autorisation saveAutorisation(String provenance, Date dateEmbarquement) {
        ...
        logger.info("Type autorisation ", env.getProperty("message.type.autorisation.ref.consommation"));
        Optional<TypeAutorisation> optionalTypeAutorisation = typeAutorisationRepository.
                findByRef(env.getProperty("message.type.autorisation.ref.consommation"));
