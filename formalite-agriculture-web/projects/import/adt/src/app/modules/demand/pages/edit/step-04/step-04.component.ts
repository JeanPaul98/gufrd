...
export class Step04Component {
  ...
  step1LocalData: NavireInfoModel = JSON.parse(
    localStorage.getItem('step1EditADT') as string
  );
  step2LocalData: { detAutorisationProduitDtos: ProductModel[] } = JSON.parse(
    localStorage.getItem('step2EditADT') as string
  );
  ...
  editDemand() {
    this.createService
      .editDemande(this.demandADT)
      .pipe(
        untilDestroyed(this),
        finalize(() => {
          localStorage.removeItem('step1EditADT');
          localStorage.removeItem('step2EditADT');
          localStorage.removeItem('currentStepEditADT');
          this.createService.isEndGoBackSubject.next(1);
        })
      )
      .subscribe({});
