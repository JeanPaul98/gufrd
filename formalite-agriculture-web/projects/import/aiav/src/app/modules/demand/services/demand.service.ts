...
export class DemandService {
  ...
  activeLink:string = localStorage.getItem('currentTabsAIAV') ?? "not-submitted";
  ...
  back() {
    ...
    this.activeLink = localStorage.getItem('currentTabsAIAV') ?? "not-submitted";
  ...
  getAgreementList(): Observable<AgreementModel[] | undefined> {
    return this.httpDemandService.getAgreementList().pipe(
      map((res) => res.result),
      catchError((err) => {
        return throwError(() => err);
      }),
