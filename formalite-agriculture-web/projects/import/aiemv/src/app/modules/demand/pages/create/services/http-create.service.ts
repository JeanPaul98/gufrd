...
export class HttpCreateService extends BaseHttpService {
  ...
  createDemande(demandInfos: DemandAeaaSimpleModel): Observable<HttpResponseModel<number>> {
    return this.http.post<HttpResponseModel<number>>(`${this.API_USERS_URL}/v1/autorisation/medicament/insert`, demandInfos, {
      headers: this.httpHeader()
    });

  }
  editDemande(demandInfos: DemandAeaaSimpleModel): Observable<HttpResponseModel<DemandAeaaSimpleModel>> {
    return this.http.post<HttpResponseModel<DemandAeaaSimpleModel>>(`${this.API_USERS_URL}/v1/autorisation/medicament/validerdemande
`, demandInfos, {
      headers: this.httpHeader()
    });
  }
