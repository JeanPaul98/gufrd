...
export class DemandComponent implements OnInit, OnDestroy {
  ...
  activeLink: string = localStorage.getItem('currentTabsIPN') ?? "not-submitted";
  ...
  listenerRouteChange() {
    this.routerSubscription = this.router.events
      ...
      .subscribe((event: any) => this.onRouteChange(event.urlAfterRedirects));
  }

  onClickNav(nav: string): void {
    this.activeLink = nav;
    this.router.navigate(["demand", nav]);
    localStorage.setItem('currentTabsIPN', nav);
    this.getBadgeCount();
  }
  ...
  ngOnDestroy(): void {
    ...
    localStorage.removeItem('currentTabsIPN');
  }
