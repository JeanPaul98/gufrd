...
import {BehaviorSubject, catchError, finalize, map, Observable, of, ReplaySubject, switchMap, throwError} from 'rxjs';
import {
...
export class DemandService {
  ...
  getDemandAccepted(): Observable<DemandAeaaModel[] | undefined> {
    return this.httpService.accepted().pipe(
      map((res) => res.result),
      switchMap((res) => {
        const demandWithUrl = res.map((demand: DemandAeaaModel) => {
          this.getPaymentUrl(demand.idFormalite).pipe(
            map((resUrl) => {
  ...
  deleteDemand(submitDemandModel: SubmitDemandModel): Observable<DemandAeaaModel | undefined> {
    return this.httpService.delete(submitDemandModel).pipe(
      map((res) => res.result),
      catchError((err) => {
        toast.custom(NiceToastComponent, {
          position: 'top-center',
  ...
  getPaymentUrl(idFormality: number): Observable<PaiementUrlModel | undefined> {
    return this.httpService.paymentUrl(idFormality).pipe(
      map((res) => res.result),
      catchError((err) => {
        return throwError(() => err);
      }),
