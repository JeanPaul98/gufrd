...
export class EditComponent implements OnInit, OnDestroy {
  items = [1, 2, 3, 4];
  fileName = '';
  currentStep = JSON.parse(localStorage.getItem('currentStepEditIPPPP') as string) ?? 1;
  ...
  onClickNext() {
    if (this.currentStep < this.steps.length) {
      this.currentStep++;
      localStorage.setItem('currentStepEditIPPPP', JSON.stringify(this.currentStep))
  ...
  ngOnDestroy(): void {
    localStorage.removeItem('step1EditIPPPP');
    localStorage.removeItem('step2EditIPPPP');
    localStorage.removeItem('currentStepEditIPPPP');
  }
}
