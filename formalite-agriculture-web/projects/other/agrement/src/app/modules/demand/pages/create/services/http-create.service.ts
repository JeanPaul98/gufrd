...
export class HttpCreateService extends BaseHttpService {
  ...
  createDemande(demandInfos: DemandAgrementModel): Observable<HttpResponseModel<number>> {
    return this.http.post<HttpResponseModel<number>>(`${this.API_USERS_URL}/v1/demande/agrement/insert`, demandInfos, {
      headers: this.httpHeader()
    });

  }
  editDemande(demandInfos: DemandAeaaSimpleModel): Observable<HttpResponseModel<DemandAeaaSimpleModel>> {
    return this.http.post<HttpResponseModel<DemandAeaaSimpleModel>>(`${this.API_USERS_URL}/v1/autorisation/depotage/validerdemande
`, demandInfos, {
      headers: this.httpHeader()
    });
  }
