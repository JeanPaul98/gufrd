...
export class EditComponent implements OnInit, AfterViewChecked {
  items = [1, 2, 3, 4];
  fileName = '';
  currentStep = JSON.parse(localStorage.getItem('currentStepEditAGREMENT') as string) ?? 1;
  ...
  onClickBack() {
    // this.currentStep = 1;
    // this.onClickPrevious()
    // localStorage.setItem('currentStepEditAGREMENT', JSON.stringify(this.currentStep))
    this.demandService.isHideDemandBarSubject.next(false);
