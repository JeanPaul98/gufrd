...
import {formatDateBK} from "../../../../../../../../../../src/app/utils/app.utils";
...
export class Step01Component implements OnInit {
  ...
  initForm(): void {
    this.step1InfoNavireFrom = this._formBuilder!.group({
      ...
      nomImportateur: [this.demand.nomImportateur, Validators.required],
      affreteur: [this.demand.affreteur, Validators.required],
      numeroAutorisation: [this.demand.numeroAutorisation, Validators.required],
      numeroBL: [this.demand.numeroBL, Validators.required],
    ...
    this.step1InfoNavireFrom.get('datearrivee')?.setValue(formatDateBK(this.demand.datearrivee));
    this.step1InfoNavireFrom.patchValue(JSON.parse(localStorage.getItem('step1EditAGREMENT') as string) ?? {});
  ...
  onClickNext() {
    ...
    localStorage.setItem('step1EditAGREMENT', JSON.stringify(this.step1InfoNavireFrom.value))
