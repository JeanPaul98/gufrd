...
import {BehaviorSubject, catchError, finalize, Observable, of, switchMap, throwError} from "rxjs";
...
import {DemandAeaaCompletModel} from "../../../../../../../../src/app/models/demand-ipn.model";
...
export class DemandService {
  ...
  getSocietes(): Observable<SocieteModel[] | undefined> {
    return this.httpDemandService.getSocietes().pipe(
      map((res) => res.result),
      catchError((err) => {
  ...
  getAgrements(): Observable<TypeAgrementModel[] | undefined> {
    return this.httpDemandService.getAgrements().pipe(
      map((res) => res.result),
      catchError((err) => {
        return throwError(() => err);
      }),
