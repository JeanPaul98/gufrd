...
export class DemandComponent implements OnInit, OnDestroy {
  ...
  activeLink:string = localStorage.getItem('currentTabsCSPCAdmin') ?? "submitted";
  ...
  ngOnInit(): void {
    this.router.navigate(['exportations/cspc/demand', this.activeLink])
  ...
  onClickNav(nav: string): void {
    this.activeLink = nav;
    this.router.navigate(["exportations/cspc/demand", nav]);
    localStorage.setItem('currentTabsCSPCAdmin', nav);
    this.getBadgeCount();
  }
