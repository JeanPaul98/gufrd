...
export class DemandComponent implements OnInit, OnDestroy {
  ...
  activeLink:string = localStorage.getItem('currentTabsCSCPAdmin') ?? "submitted";
  ...
  ngOnInit(): void {
    this.router.navigate(['exportations/cscp/demand', this.activeLink])
  ...
  onClickNav(nav: string): void {
    this.activeLink = nav;
    this.router.navigate(["exportations/cscp/demand", nav]);
    localStorage.setItem('currentTabsCSCPAdmin', nav);
    this.getBadgeCount();
  }
