...
export class DemandService {
  ...
  back() {
    // this.location.back();
    this.router.navigate(['/exportations/cscp/demand', 'submitted'])
  ...
  generateCertificat(idFormalite:number): Observable<string | undefined> {
    return this.httpDemandService.generateCertificat(idFormalite).pipe(
      map((res) => res),
      catchError((err) => {
        return throwError(() => err);
      }),
      ...
      map((blob) => {
        ...
        a.href = url;
        a.download = `FORMALITE_${idFormalite}_CERTIFICAT_CIRE.pdf`;
