...
export class DemandComponent implements OnInit, OnDestroy {
  ...
  activeLink:string = localStorage.getItem('currentTabsIOCAVAdmin') ?? "submitted";
  ...
  ngOnInit(): void {
    this.router.navigate(['exportations/iocav/demand', this.activeLink])
  ...
  onClickNav(nav: string): void {
    this.activeLink = nav;
    this.router.navigate(["exportations/iocav/demand", nav]);
    localStorage.setItem('currentTabsIOCAVAdmin', nav);
    this.getBadgeCount();
  }
