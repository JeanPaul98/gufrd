...
export class DemandComponent implements OnInit, OnDestroy {
  ...
  activeLink:string = localStorage.getItem('currentTabsADTAdmin') ?? "submitted";
  ...
  ngOnInit(): void {
    this.router.navigate(['importations/adt/demand', this.activeLink])
  ...
  onClickNav(nav: string): void {
    this.activeLink = nav;
    this.router.navigate(["importations/adt/demand", nav]);
