...
export class DemandComponent implements OnInit, OnDestroy {
  ...
  activeLink:string = localStorage.getItem('currentTabsAIAVAdmin') ?? "submitted";
  ...
  ngOnInit(): void {
    this.router.navigate(['importations/aiav/demand', this.activeLink])
  ...
  onClickNav(nav: string): void {
    this.activeLink = nav;
    this.router.navigate(["importations/aiav/demand", nav]);
    localStorage.setItem('currentTabsAIAVAdmin', nav);
    this.getBadgeCount();
  }
