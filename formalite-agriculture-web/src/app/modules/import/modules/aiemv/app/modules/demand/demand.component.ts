...
export class DemandComponent implements OnInit, OnDestroy {
  ...
  activeLink:string = localStorage.getItem('currentTabsAIEMVAdmin') ?? "submitted";
  ...
  ngOnInit(): void {
    this.router.navigate(['importations/aiemv/demand', this.activeLink])
  ...
  onClickNav(nav: string): void {
    this.activeLink = nav;
    this.router.navigate(["importations/aiemv/demand", nav]);
    localStorage.setItem('currentTabsAIEMVAdmin', nav);
    this.getBadgeCount();
  }
