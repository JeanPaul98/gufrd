...
export class DemandComponent implements OnInit, OnDestroy {
  ...
  activeLink:string = localStorage.getItem('currentTabsAIPCLAdmin') ?? "submitted";
  ...
  ngOnInit(): void {
    this.router.navigate(['importations/aipcl/demand', this.activeLink])
  ...
  onClickNav(nav: string): void {
    this.activeLink = nav;
    this.router.navigate(["importations/aipcl/demand", nav]);
    localStorage.setItem('currentTabsAIPCLAdmin', nav);
    this.getBadgeCount();
  }
