...
export class DemandComponent implements OnInit, OnDestroy {
  ...
  activeLink:string = localStorage.getItem('currentTabsIPCCAdmin') ?? "submitted";
  ...
  onClickNav(nav: string): void {
    this.activeLink = nav;
    this.router.navigate(["importations/ipcc/demand", nav]);
    localStorage.setItem('currentTabsIPCCAdmin', nav);
    this.getBadgeCount();
  }
