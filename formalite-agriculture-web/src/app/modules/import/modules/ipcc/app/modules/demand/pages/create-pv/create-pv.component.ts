...
export class CreatePvComponent implements OnInit {
  currentStep = JSON.parse(localStorage.getItem('currentStepPVIPCC') as string) ?? 1;
  ...
  steps: StepModel[] = [
    ...
    {title: 'Etape 4', description: 'Informations sur traitements produits'},
    {title: 'Etape 5', description: 'Recapitulatif'},
  ];
  ...
  ngOnInit(): void {
    ...
    this.router.navigate(['/importations/ipcc/demand/create-pv', `step${this.currentStep}`])
  ...
  onClickNext(){
    if(this.currentStep < this.steps.length ) {
      this.currentStep++;
      localStorage.setItem('currentStepPVIPCC', JSON.stringify(this.currentStep))
  ...
  onClickBack() {
    ...
    // this.demandService.customBack('/importations/ipcc/demand/'+this.curentTab);
  }
}
