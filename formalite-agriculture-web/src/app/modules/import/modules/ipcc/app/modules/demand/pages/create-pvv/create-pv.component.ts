...
export class CreatePvComponent implements OnInit {
  ...
  steps: StepModel[] = [
    ...
    {
      title: 'Etape 2',
      description: 'Marchandises Reperees et Mesures Prescrites',
    },
  ...
  ngOnInit(): void {
    ...
    this.router.navigate([
      '/importations/ipcc/demand/create-pv',
      `step${this.currentStep}`,
    ]);

    this.stepperService.nowGoToNext
      .pipe(untilDestroyed(this), debounceTime(300))
      .subscribe(() => {
        this.onClickNext();
      });

    this.stepperService.nowGoToPrevious
      .pipe(untilDestroyed(this), debounceTime(300))
      .subscribe(() => {
        this.onClickPrevious();
      });

    this.createPVService.isEndGoBackSubject
      .pipe(untilDestroyed(this), debounceTime(300))
      .subscribe((res) => {
        if (res === 1) {
          this.goBackToBegin();
