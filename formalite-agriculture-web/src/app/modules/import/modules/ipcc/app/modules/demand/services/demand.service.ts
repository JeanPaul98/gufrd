...
export class DemandService {
  ...
  activeLink:string = localStorage.getItem('currentTabsIPCCAdmin') ?? "not-submitted";
  ...
  constructor(private router: Router, private location: Location, private httpDemandService: HttpDemandService) {
    ...
    back() {
    // this.location.back();
    this.router.navigate(['/importations/ipcc/demand', this.activeLink])
  ...
  generatePVPdf(idPv:number): Observable<string | undefined> {
    return this.httpDemandService.generatePVPdf(idPv).pipe(
      map((res) => res),
      catchError((err) => {
        return throwError(() => err);
      }),
      ...
      map((blob) => {
        ...
        a.href = url;
        a.download = `PV_${idPv}_PHYTO_CARGO_CONTAINER.pdf`;
