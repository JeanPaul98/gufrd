...
export class DemandComponent implements OnInit, OnDestroy {
  ...
  activeLink:string = localStorage.getItem('currentTabsIPNAdmin') ?? "submitted";
  ...
  onClickNav(nav: string): void {
    this.activeLink = nav;
    this.router.navigate(["importations/ipn/demand", nav]);
    localStorage.setItem('currentTabsIPNAdmin', nav);
    this.getBadgeCount();
  }
