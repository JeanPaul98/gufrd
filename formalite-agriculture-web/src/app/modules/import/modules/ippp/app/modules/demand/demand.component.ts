...
export class DemandComponent implements OnInit, OnDestroy {
  ...
  activeLink:string = localStorage.getItem('currentTabsIPPPAdmin') ?? "submitted";
  ...
  onClickNav(nav: string): void {
    this.activeLink = nav;
    this.router.navigate(["importations/ippp/demand", nav]);
    localStorage.setItem('currentTabsIPPPAdmin', nav);
    this.getBadgeCount();
  }
