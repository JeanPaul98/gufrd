...
@Component
public class JwtAdminAuthConverter implements Converter<Jwt, AbstractAuthenticationToken> {

    private final Logger logger = LoggerFactory.getLogger(JwtAdminAuthConverter.class);


    private final JwtGrantedAuthoritiesConverter jwtGrantedAuthoritiesConverter =
            new JwtGrantedAuthoritiesConverter();

    @Value("${jwt.auth.client.admin.resource-id}")
    ...
    public AbstractAuthenticationToken convert(@Nonnull Jwt jwt) {
        Collection<GrantedAuthority> authorities = Stream.concat(jwtGrantedAuthoritiesConverter.convert(jwt).stream(), extractRessourceRole(jwt).stream())
    ...
    private KeyCloakUser getKeycloakUser(Jwt jwt) {
        ...
        logger.info("keycloakUser client admin local , {}", user);
