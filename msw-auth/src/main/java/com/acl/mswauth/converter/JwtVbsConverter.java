 ...
 */
@Component
public class JwtVbsConverter implements Converter<Jwt, AbstractAuthenticationToken> {

    private final Logger logger = LoggerFactory.getLogger(JwtVbsConverter.class);


    private final JwtGrantedAuthoritiesConverter jwtGrantedAuthoritiesConverter =
            new JwtGrantedAuthoritiesConverter();

    @Value("${jwt.auth.client.vbs.resource-id}")
    ...
    private KeyCloakUser getKeycloakUser(Jwt jwt) {
        ...
        logger.info("Infos utilisateur , {} , {}", email, name);
        KeyCloakUser user = new KeyCloakUser(name, fullname, email, name);
        logger.info("keycloakUser  vbs, {}", user);
